 ...
 *
 */
public class InteractivePottsViewer extends JComponent {
	 ...
	 */
	public InteractivePottsViewer(InteractivePotts attachedTo) {
	...
	protected void paintComponent(Graphics g) {
		...
		for(int i=0;i<N;i++){
			for(int j=0;j<N;j++){
				if(controller.getSpinState(i,j) == 0){
					g.setColor(Color.GREEN);
				}else if(controller.getSpinState(i,j) == 1){
					g.setColor(Color.RED);
				}else if (controller.getSpinState(i,j) == 2){
